...
def parse():
    ...
    url='https://api-selectel.pik-service.ru/v2/filter?type=1,2&location=2,3&block=294&flatPage=3&flatLimit=8&onlyFlats=1'
    ...
    for i in range(1,count_pages+1):
        url = 'https://api-selectel.pik-service.ru/v2/filter?type=1,2&location=2,3&block=294&flatPage=' + str(i) +'&flatLimit=8&onlyFlats=1'
        req = requests.get(url, headers=headers)
        src = req.text
        data = json.loads(src)
        for m in range(8):
    ...
    writer = pd.ExcelWriter(f'data/{str(date.today())}_pik_bd_park.xlsx')
    ...
    '*Данные взяты с сайта pik.ru/bd.')
